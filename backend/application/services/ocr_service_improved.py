...
class OCRService:
    ...
    def _extract_character_regions_improved(self, image: np.ndarray) -> List[Tuple[int, int, int, int]]:
        """改善された文字領域抽出 - 近接する輪郭を結合"""
        # より強力なモルフォロジー処理で文字部分を結合
        ...
        while i < len(bboxes):
            ...
            if i + 1 < len(bboxes):
                ...
                # X方向の距離が近く、Y方向の重なりがある場合は結合
                x_distance = x2 - (x1 + w1)
                y_overlap = min(y1 + h1, y2 + h2) - max(y1, y2)
                
                if x_distance < 20 and y_overlap > 0:  # 20ピクセル以内なら同じ文字の可能性
                    # 結合したバウンディングボックス
                    new_x = min(x1, x2)
                    new_y = min(y1, y2)
                    ...
                    i += 2  # 2つのボックスを処理したので2つ進める
                    merged = True
            
            if not merged:
                merged_bboxes.append((x1, y1, w1, h1))
                i += 1
